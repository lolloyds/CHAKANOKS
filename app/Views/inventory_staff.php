...
<main>
	<style>
		...
		.stat-card { background:#fff; border:1px solid #eee; border-radius:8px; padding:12px; text-align:center; }
	...
	<div class="box">
		...
		<div class="desc">Track inventory in real-time, adjust quantities, and scan barcodes.</div>
	</div>
	...
	<div class="grid grid-2">
		<div class="box">
			...
			<div class="row">
				<div><button id="btn-start">Start Scanner</button></div>
				<div><button id="btn-stop">Stop Scanner</button></div>
			</div>
			<p class="muted">Allow camera access, then scan a SKU to auto-filter items below.</p>
		</div>
		<div class="box">
			<h3>🔎 Search</h3>
			<input type="search" id="search" placeholder="Search by name or SKU" />
			<p class="muted">Type or scan to filter the table. Click Apply to save stock changes.</p>
		</div>
	</div>
...
<?php include __DIR__ . '/includes/footer.php'; ?>
	<script>
		...
		function render(items) {
			...
			for (const it of filtered) {
				...
				tr.innerHTML = `
					...
					<td>
						<svg id="barcode-${it.id}" class="barcode"></svg>
					</td>
					<td>
						<div class="row">
							<div class="col-sm-4"><input type="number" id="chg-${it.id}" placeholder="e.g. 5 or -2" /></div>
							<div class="col-sm-4"><input type="text" id="rsn-${it.id}" placeholder="Reason (optional)" /></div>
							<div class="col-sm-4"><button data-id="${it.id}">Apply</button></div>
						</div>
					</td>
				`;
		...
		function onScanSuccess(decodedText) {
			...
			refresh();
			// Try to focus matched row's adjust input
			const items = window.__lastItems || [];
			const match = items.find(it => it.sku.toLowerCase() === decodedText.trim().toLowerCase());
			if (match) {
				setTimeout(() => {
					const el = document.getElementById(`chg-${match.id}`);
					if (el) {
